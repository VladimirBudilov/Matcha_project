...
)
{
	public async Task Seed()
	{
		...
		var defaultPicture = new Picture()
		{
			Id = 501,
			UserId = 0,
			PicturePath = await File.ReadAllBytesAsync(Path.Combine(projectDirectory, "default.jpeg")),
			IsProfilePicture = true
		};
		...
		await UpdateSequenceValue("users_user_id_seq", "user_id", "users");
		await UpdateSequenceValue("profiles_profile_id_seq", "profile_id", "profiles");
		await UpdateSequenceValue("pictures_picture_id_seq", "picture_id", "pictures");
		await UpdateSequenceValue("interests_interest_id_seq", "interest_id", "interests");
	}
