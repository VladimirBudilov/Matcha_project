...
const profile = ref<Profile>({
  ...
  "pictures": [],
  "interests": [],
  "isOnlineUser": false, // Add this property
  "lastLogin": null // Add this property if needed
});

const GetProfile = async () => {
  await axios.get('/api/profiles/' + route.params.id)
    .catch(async (error) => {
      console.error('Error response:', error.response);
      if (error.response && error.response.status === 403) {
        message.error('Access forbidden: You do not have permission to view this profile.');
      } else {
...
<template>
  <a-card id="User">
    ...
    >
      <div class="Main-info">
        ...
        <a-form-item label="Online Status">
          <p v-if="profile.isOnlineUser" style="color: green;">Online</p>
          <p v-else style="color: red;">
            Offline
            <span style="color: grey;" v-if="profile.lastLogin">
              last was seen {{ dayjs(profile.lastLogin).format('YYYY-MM-DD HH:mm:ss') }}
            </span>
          </p>
        </a-form-item>
      </div>
  ...
  <a-card id="Avatar">
    <a-form-item label="Avatar">
      <a-image v-if="profile.profilePicture.picture" :src="'data:image/*' + ';base64,' + profile.profilePicture.picture" />
    </a-form-item>
  </a-card>
  ...
  <a-card id="Photos">
    <a-form-item label="Photos">
      <a-image-preview-group v-for="item in profile.pictures">
        <a-image :src="'data:image/*' + ';base64,' + item.picture" />
